...
def generate_times(df, T=None, T_to_death=None, seed=None, censor=False):
    ...
    if T_to_death is not None:
        ...
        df.loc[idxInICU, "windowtime"] = (
            df.loc[idxInICU, "deathtime_hours"] - T_to_death
        )
...
def print_cm(y, yhat):
    ...
    print("y=0\t{:6g}\t{:6g}\tSpec={:2.2f}").format(
        cm[0, 0], cm[0, 1], 100.0 * TN / (TN + FP)
    )  # Spec
    print("y=1\t{:6g}\t{:6g}\tSens={:2.2f}").format(
        cm[1, 0], cm[1, 1], 100.0 * TP / (TP + FN)
    )  # Sens
...
def vars_of_interest():
    ...
    var_min = ["heartrate", "sysbp", "diasbp", "meanbp", "resprate", "tempc", "spo2"]
    var_max = var_min
    var_min.append("gcs")
    # var_max.extend(['rrt','vasopressor','vent'])

    # we extract the first/last value for these covariates
    var_first = ["heartrate", "sysbp", "diasbp", "meanbp", "resprate", "tempc", "spo2"]
    ...
    var_first_early = [
        ...
        "bg_pao2fio2ratio",
        "bg_ph",
        "bg_baseexcess",  #'bg_bicarbonate',
        "bg_totalco2",  #'bg_hematocrit', 'bg_hemoglobin',
        "bg_carboxyhemoglobin",
        "bg_methemoglobin",
...
def vars_of_interest_streaming():
    ...
    var_first_early = [
        "bg_so2",
        "bg_po2",
        "bg_pco2",  #'bg_fio2_chartevents', 'bg_aado2_calc',
        "bg_fio2",
        "bg_aado2",
        "bg_pao2fio2",
        "bg_ph",
...
def get_design_matrix(df, time_dict, W=8, W_extra=24):
    ...
    tmp = np.array(list(time_dict.items()))
    N = tmp.shape[0]
    ...
    df_time = pd.DataFrame(
        data=tmp, index=None, columns=["icustay_id", "hr", "early_flag"]
    )
    ...
    df = df.merge(
        df_time,
        left_on=["icustay_id", "hr"],
        right_on=["icustay_id", "hr"],
        how="inner",
    )
    ...
    df_data = pd.concat(
        [df_first, df_first_early, df_last, df_last_early, df_min, df_max, df_sum],
        axis=1,
    )
...
def debug_for_iid(df, time_dict, iid, T=8, W_extra=24):
    ...
    display(
        HTML(df_time.loc[df_time["icustay_id"] == iid].to_html().replace("NaN", ""))
    )
    ...
    display(
        HTML(
            df_tmp.groupby("icustay_id")[var_first].first().to_html().replace("NaN", "")
        )
    )
    print("\n\nLAST\n\n")
    display(
        HTML(
            df_tmp.groupby("icustay_id")[var_first].last().to_html().replace("NaN", "")
        )
    )
...
def collapse_data(data):
    ...
    colNameMap = {"vent": "vent", "vasopressor": "vasopressor", "rrt_range": "rrt"}
    ...
    for i, f in enumerate(files):
        ...
        if initFlag == False:
            ...
            df = df.merge(df_tmp, on=["icustay_id", "charttime_elapsed"], how="outer")

        return df


def plot_xgb_importance_fmap(
    xgb_model,
    X_header=None,
    ax=None,
    height=0.2,
...
def plot_vitals(df, iid, df_death=None, df_censor=None):
    ...
    plt.plot(
        df.loc[idx, "hr"], df.loc[idx, "sysbp"], "b^", label="Systolic BP", alpha=0.5
    )
    plt.plot(
        df.loc[idx, "hr"], df.loc[idx, "diasbp"], "bv", label="Diastolic BP", alpha=0.5
    )
    ...
    if df_death is not None:
        ...
        plt.plot(
            ...
            * df_death.loc[idx, "dischtime_hours"].values,
            [0, 200],
            "k--",
            linewidth=2,
        ...
        if df_death.loc[idx, "deathtime_hours"] is not np.nan:
            plt.plot(
                ...
                * df_death.loc[idx, "deathtime_hours"].values,
                [0, 200],
                "k-",
                linewidth=2,
                label="Time of death",
    ...
    if df_censor is not None:
        ...
        if np.any(idx):
            plt.plot(
                ...
                * df_censor.loc[idx, "censortime_hours"].values,
                [0, 200],
                "m--",
                alpha=0.8,
                linewidth=3,
...
def load_design_matrix(
    co, df_additional_data=None, data_ext="", path=None, diedWithin=None
):
    ...
    df_offset = df_offset.loc[
        :, ["icustay_id", "intime", "outtime", "starttime", "deathtime"]
    ]
    ...
    if diedWithin is not None:
        df = df.merge(
            df_offset[["intime", "deathtime", "starttime"]],
            how="left",
            left_index=True,
            right_index=True,
        )
    ...
    vars_to_delete = [
        "bg_intubated_first",
        "bg_ventilationrate_first",
        "bg_ventilator_first",
        "bg_intubated_last",
        "bg_ventilationrate_last",
        "bg_ventilator_last",
        "rrt_min",
        "vasopressor_min",
        "vent_min",
