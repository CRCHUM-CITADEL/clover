...
The only hyperparameter of Synthpop is the order of the variables sequence to be synthesized. The default visit sequence is the dataset columns order. Another option would be to conceptualize the variables order as an optimization problem such as the Traveling Salesman Problem. The discrete Particule Swarm Optimization (PSO) implemented in the library can be used to solve the problem. For more details, please refer to the `discrete PSO section <../optimization/pso.rst>`_ in the User Guide and to the notebook `Tune hyperparameters <../../tutorial/tune_hyperparameters.nblink>`_ to learn how to use the optimizer.
