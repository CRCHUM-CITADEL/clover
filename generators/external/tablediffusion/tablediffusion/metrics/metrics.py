...
def _get_frequencies(real, synthetic):
    ...
    """
    f_obs, f_exp = [], []
    real, synthetic = defaultdict(float, Counter(real)), defaultdict(
        float, Counter(synthetic)
    )
    ...
    for _, (col, var_type) in enumerate(data_types):
        if "categorical" in var_type or "int" in var_type:
            ...
            statistic, _ = ks_2samp(
                real_dataset[col].fillna(0), fake_dataset[col].fillna(0)
            )
            result = statistic
        else:
...
def pmse_ratio(data, synthetic_data):
    ...
    expected_utility = (
        clf.coef_.shape[1] * (n1 / (n1 + n2)) ** 2 * (n2 / (n1 + n2)) / (n1 + n2)
    )
...
def sra(real, synth):
    ...
    for i in range(k):
        R_vals = np.array(
            [real[i] - rj if i != k else None for k, rj in enumerate(real)]
        )
        S_vals = np.array(
            [synth[i] - sj if i != k else None for k, sj in enumerate(synth)]
        )
