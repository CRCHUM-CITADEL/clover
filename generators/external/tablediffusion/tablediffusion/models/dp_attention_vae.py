...
class DPattentionVAE_Synthesiser:
    ...
    def fit(
        ...
        **kwargs,
        ...
        self.con_idxs = [
            i for i, _ in enumerate(self.colnames) if i not in self.cat_idxs
        ]
        ...
        if self._elapsed_batches == 0:
            ...
            self.enc = Encoder(self.embedding_size, self.enc_dims, self.latent_dim).to(
                self.device
            )
            self.dec = Decoder(self.latent_dim, self.dec_dims, self.embedding_size).to(
                self.device
            )
            ...
            self.optim_trans = optim.AdamW(
                self.AE.transformer.parameters(), lr=self.lr_trans
            )
            ...
            if self.mlflow_logging:
                ...
                mlflow.log_params(_param_dict)
                _param_dict = gather_object_params(
                    self.optim_mlp1, prefix="optim_mlp1."
                )
                mlflow.log_params(_param_dict)
                _param_dict = gather_object_params(
                ...
                mlflow.log_params(_param_dict)
                _param_dict = gather_object_params(
                    self.optim_mlp2, prefix="optim_mlp2."
                )
                mlflow.log_params(_param_dict)
        ...
        for epoch in range(n_epochs):
            ...
            for i, data in enumerate(trainloader, 0):
                ...
                if i % 10 == 0 and self.mlflow_logging:
                    ...
                    mlflow.log_metrics(
                        calc_norm_dict(self.AE), step=self._elapsed_batches
                    )
                    mlflow.log_metrics(
                        calc_norm_dict(self.enc), step=self._elapsed_batches
                    )
                    mlflow.log_metrics(
                        calc_norm_dict(self.dec), step=self._elapsed_batches
                    )
    ...
    def sample(self, n=None):
        ...
        Tensor = torch.cuda.FloatTensor if self.cuda else torch.FloatTensor
        cat_samples, con_samples = [[] for i in self.cat_idxs], [
            [] for i in self.con_idxs
        ]
        ...
        cat_samples = [
            self.cat_label_encs[i].inverse_transform(c)
            for i, c in enumerate(cat_samples)
        ]
        ...
        df[self.numeric_cols] = df[self.numeric_cols].apply(
