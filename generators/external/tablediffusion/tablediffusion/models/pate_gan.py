...
class PATEGAN_Synthesiser:
    ...
    def fit(self, data, n_epochs=10, epsilon=100, noise_multiplier=1e-3):
        ...
        if self._elapsed_batches == 0:
            ...
            self.D = (
                Discriminator(self.data_dim, dis_dims=self.dis_dims)
                .double()
                .to(self.device)
            )
            ...
            if self.mlflow_logging:
                ...
                _param_dict = gather_object_params(
                    self.optimiser_t[0], prefix="optim_D."
                )
                mlflow.log_params(_param_dict)
        ...
        for epoch in range(n_epochs):
            ...
            for _ in range(self.student_iters):
                ...
                predictions, votes = pate(
                    fake_data, self.teacher_disc, noise_multiplier
                )
            ...
            if self.mlflow_logging:
                mlflow.log_metrics(
                    {"used_epsilon.total": self._eps}, step=self._elapsed_batches
                )

            if float(self._eps) >= self._epsilon:
                print(
                    f"Privacy budget reached before training G in epoch {epoch} ({self._eps})"
                )
                return self
