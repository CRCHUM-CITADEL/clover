...
from ..utilities.saint_utils import (
    RowColTransformer,
    Transformer,
    embed_data_mask,
    mdn_loss,
...
def train_on_batch(
    ...
    x_categ, x_cont, x_categ_enc_2, x_cont_enc_2 = embed_and_mask(
        data, model=model, device=device
    )
...
def samples_to_df(
    cat_samples, con_samples, cat_idxs, colnames, cat_label_encs, datatypes=None
):
    ...
    cat_samples = [
        cat_label_encs[i].inverse_transform(c) for i, c in enumerate(cat_samples)
    ]
