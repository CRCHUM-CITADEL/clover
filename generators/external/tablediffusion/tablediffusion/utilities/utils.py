...
def gather_object_params(obj, prefix="", clip=249):
    # TODO docstring
    return {
        prefix + k: str(v)[:clip] if len(str(v)) > clip else v
...
def run_synthesisers(
    ...
    for repeat in tqdm(range(1, repeats + 1), desc="Repeats", leave=True, colour="red"):
        ...
        for dataset, data_params in datasets.items():
            ...
            X, _X, processor = load_and_prep_data(
                dataset=dataset, datadir=datadir, verbose=False
            )
            disc_cols = [
                c for c, dtype in data_params["data_types"] if "categorical" in dtype
            ]
            ...
            if with_benchmark and fake_data_path is not None:
                ...
                pd.DataFrame(X_fake_benchmark).to_csv(
                    Path(fake_data_path)
                    / f"fake_{dataset}_CTGAN_Synthesiser_0.0_{repeat}.csv",
                    index=False,
                )
                    ...
                    with mlflow.start_run(run_name=run_name, experiment_id=exp_id):
                        ...
                        try:
                            ...
                            mlflow.log_metrics(
                                {
                                    "epsilon": model._eps,
                                    "elapsed_batches": model._elapsed_batches,
                                },
