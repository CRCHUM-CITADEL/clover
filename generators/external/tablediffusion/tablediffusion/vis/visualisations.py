...
def visualise_metrics(
    data: pd.DataFrame,
    use_log=False,
    benchmark_synth="CTGAN_Synthesiser",
    emph_synth=None,
):
    """
    ...
    for j, dataset in enumerate(datasets):
        for i, metric in enumerate(metrics):
            ...
            for k, synth in enumerate(_data["synth"].unique()):
                _subset = _data[
                    (_data["dataset"] == dataset) & (_data["synth"] == synth)
                ]
                _subset = (
...
def visualise_marginals(
    real_dataset: pd.DataFrame, fake_dataset: pd.DataFrame, data_types
):
    """
    ...
    for i, (col, var_type) in enumerate(data_types):
        ...
        if "int" in var_type or "float" in var_type:
            ...
            bin_width = (
                2 * iqr * len(real_dataset[col].dropna()) ** (-1 / 3) if iqr != 0 else 1
            )
            bins = min(
                int((real_dataset[col].max() - real_dataset[col].min()) / bin_width), 50
            )
            ...
            # Overlay KDE
            sns.kdeplot(
                real_dataset[col], ax=axes[0], color="black", warn_singular=False
            )
            sns.kdeplot(
                fake_dataset[col], ax=axes[1], color="black", warn_singular=False
            )
...
def visualise_pca(real_dataset, fake_dataset=None, mult=1.2, legend=False):
    ...
    with warnings.catch_warnings():
        ...
        density = ax.scatter_density(
            _data[:, 0], _data[:, 1], cmap=plt.cm.hot, norm=norm
        )
